...
win11 = tk.Tk()
win11.title("Search Window")

pic= PhotoImage(master=win11,file="BuySellCar.png")
pic_label = Label(win11,image=pic).grid(column=7,row=0,columnspan=10, rowspan=20)

def check():
 
    f = ttk.LabelFrame(win11)
    ...
    cursor = conn.cursor()
    cursor.execute("select rc, make, model, yr, run, colour, fuel, selling_price from inventory where stat = 0 and make = '" +str(show.get())+ "'")
...
lshow = tk.Label(rf, text="Enter Make: ",font=('times new roman',15)).grid(column=1,row=1,padx=10)

show = tk.StringVar() 
tshow = tk.Entry(rf,font=('times new roman',15), textvariable=show) 
tshow.grid(column=2, row=1)

insert=tk.Button(win11,font=('Times New Roman',15), text="Check!" ,command=check) 
...
back=tk.Button(win11,font=('Times New Roman',15), text="Back to Previous Menu",command=back)
